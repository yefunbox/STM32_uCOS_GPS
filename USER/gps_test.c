...
int mainTest()
{
    ...
    for(it = 0; it < NUM_POINTS; ++it)
    {
        ...
        printf("Azimuth  P0 to P%d\tat start: %8.3lf癨tat end: %8.3lf癨n", it, nmea_radian2degree(azimuth[it][0]), nmea_radian2degree(azimuth[it][1]));
        printf("Move     P0 to P%d\t         \tAzimuth at end: %8.3lf癨n", it, nmea_radian2degree(azimuth_moved[it]));
        printf("Move     P0 to P%d\ton spheroid:  %3s lat:%+010.6lf?lon:%+011.6lf癨n", it, result[0] == 1 ? "OK" : "nOK", nmea_radian2degree(pos_moved[it][0].lat), nmea_radian2degree(pos_moved[it][0].lon));
        printf("Move     P0 to P%d\ton ellipsoid: %3s lat:%+010.6lf?lon:%+011.6lf癨n", it, result[0] == 1 ? "OK" : "nOK", nmea_radian2degree(pos_moved[it][1].lat), nmea_radian2degree(pos_moved[it][1].lon));
        printf("Move     P0 to P%d\toriginal:         lat:%+010.6lf?lon:%+011.6lf癨n", it, nmea_radian2degree(pos[it].lat), nmea_radian2degree(pos[it].lon));
    }
